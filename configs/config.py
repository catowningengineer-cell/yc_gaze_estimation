...
def options_to_config(options: list[str], without_equal_sign: bool) -> DictConfig:
    if not options:
        return OmegaConf.create()
    if without_equal_sign:
        keys = options[::2]
        vals = options[1::2]
        options = [f"{k}={v}" for k, v in zip(keys, vals)]
    return OmegaConf.from_cli(options)


def flatten_keys(config: dict) -> list[str]:
    keys = []
    for key, value in config.items():
        if isinstance(value, dict):
            subkeys = flatten_keys(value)
            keys.extend([f"{key}.{sub}" for sub in subkeys])
        else:
            keys.append(key)
    return keys
...
def check_if_cli_keys_in_config(config: DictConfig,
    ...
    base_keys = set(flatten_keys(OmegaConf.to_container(config)))
    cli_keys = set(flatten_keys(OmegaConf.to_container(cli_config)))
